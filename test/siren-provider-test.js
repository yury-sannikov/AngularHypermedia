...
describe("Siren provider", function () {
  ...
  it('GetLinkUrlByRelVersion', function () {
  	...
  	var result = siren.GetLinkUrlByRelVersion(helperThis.data.links, "benefits/mybenefits", helperThis.proto);
  	expect(result).toBe("http://localhost:55556/api/benefits/mybenefits");

		var helperThisNoVer = {data: apiRootData, proto: null};

    	result = siren.GetLinkUrlByRelVersion(helperThisNoVer.data.links, "benefits/mybenefits");
    	expect(result).toBe("http://localhost:55556/api/2/benefits/mybenefits");

    	result = siren.GetLinkUrlByRelVersion(helperThis.data.links, "benefits/mybenefits", "0.0.1");
    	expect(result).toBe("http://localhost:55556/api/benefits/mybenefits");

    	result = siren.GetLinkUrlByRelVersion(helperThis.data.links, "benefits/mybenefits", "0.0.3");
  ...
  it('CreateEntities from requested data', inject(function ($rootScope) {
		...
		var result = {};
		siren.CreateEntities(helperThisNoVer.data.entities, result, siren.transform, "0.0.1");
